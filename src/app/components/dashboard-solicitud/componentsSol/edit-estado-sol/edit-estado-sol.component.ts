...
export class EditEstadoSolComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<EditEstadoSolComponent>,
  ...
  ngOnInit(): void {
    ...
    this.solicitudService
      .getSolicitudById(this.id_solicitud)
      .subscribe((solicitud) => {
        this.form.patchValue({
          estado: solicitud.estadosolicitud.id_estado_solicitud,
          observaciones: solicitud.observaciones, // ← parchea aquí
        });
      });
  }
              ...
              switch (body.id_estado) {
                ...
                case 2: // Aprobado
                  await this.mailService
                    .enviarEmailSolicitudAceptada(solicitud2)
                    .subscribe({
                      next: (response) => {
                        console.log(
                          'Email de APROBACIÓN enviado con éxito',
                          response
                        );
                      },
                      error: (error) => {
                ...
                case 3: // Rechazado
                  await this.mailService
                    .enviarEmailSolicitudRechazada(solicitud2)
                    .subscribe({
                      next: (response) => {
                        console.log(
                          'Email de RECHAZO enviado con éxito',
                          response
                        );
                      },
                      error: (error) => {
                        console.error(
                          'Error al enviar email de RECHAZO',
                          error
                        );
                      },
                    });
