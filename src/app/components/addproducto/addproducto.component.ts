...
export class AddproductoComponent {
  ...
  obtenerProductos(): void {
    let areaId = 1
    this.stockService.getStockByArea(areaId)
      .subscribe((data: any) => {
        this.products = data;
  ...
  async addToCart(producto: StockProducto) {
    ...
    if (!producto || producto.stock < 1) {
      console.error('No hay stock suficiente para el producto:', producto.producto.nombre);
      return;
    }
    ...
    if (!res) {
      ...
      this.carritoService.actualizarProductosSinStock(solProd.productoId.producto.id_producto, true);  
      return false;
    } else {
      // Si hay stock, asegurarse de que el ID del producto no está en la lista de sin stock
      this.carritoService.actualizarProductosSinStock(solProd.productoId.producto.id_producto, false);
      return true;
    }
