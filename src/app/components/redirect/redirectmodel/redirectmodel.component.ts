...
export class RedirectmodelComponent {
  ...
  ngOnInit(): void {
    ...
    // ✅ Delegamos al servicio, que ya tiene la configuración correcta
    this.authSSOService.resumeLoginFlow();

    // ✅ Esperamos al estado de autenticación
  ...
  public async perfil() {
    ...
    if (data) {
      ...
      if (
        this.user.area === 'duoc.cl' ||
        this.user.area === 'duocuc.cl' ||
        this.user.area === 'profesor.duoc.cl'
      ) {
        if (escuela) {
  ...
  async cliente(email: string): Promise<Observable<Clientedata>> {
    ...
    return await this.clienteService.escuelaByCorreo(email).pipe(
      ...
      catchError((error) => {
        console.log(error);
        return throwError(
          () => new Error('Error al obtener los datos del cliente')
        );
      })
    );
  }
